...
// ── Modelos MongoDB ──────────────────────────────────────────
let UserM, ChatM, ResultadoM, NotasM, RecursoM, ActividadM;
function definirModelos() {
  const S = mongoose.Schema;
  UserM      = mongoose.models.User      || mongoose.model('User',      new S({ username:{type:String,unique:true,required:true}, password:String, name:String, emoji:{type:String,default:'👤'}, role:{type:String,default:'estudiante'}, fotoUrl:String, telefono:String, fechaNacimiento:String, bio:String, intereses:String }, {timestamps:true}));
...
// ── JSON fallback ────────────────────────────────────────────
const FILES = { users:path.join(DATA_DIR,'usuarios.json'), chat:path.join(DATA_DIR,'chat.json'), resultados:path.join(DATA_DIR,'resultados.json'), notas:path.join(DATA_DIR,'notas.json'), recursos:path.join(DATA_DIR,'recursos.json'), actividad:path.join(DATA_DIR,'actividad.json') };
...
function requireAuth(req,res,next){ const token=(req.headers['authorization']||'').replace(/^Bearer\s+/i,''); if(!token) return res.status(401).json({error:'Token requerido'}); try{ req.user=jwt.verify(token,JWT_SECRET_F); next(); }catch{ res.status(401).json({error:'Token inválido o expirado.'}); } }
...
app.post('/api/recursos/enlace',requireTutor,async(req,res)=>{ const{titulo,modulo,enlace}=req.body||{}; if(!enlace) return res.status(400).json({error:'URL requerida'}); const r={id:Date.now(),titulo:(titulo||enlace).trim(),modulo:modulo||'general',tipo:'enlace',url:enlace,filename:null,uploadedBy:req.user.username,uploaderName:req.user.name,fecha:new Date().toLocaleString('es-CO',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}),ts:new Date().toISOString()}; await saveRecurso(r); res.json({ok:true,resource:r}); });
